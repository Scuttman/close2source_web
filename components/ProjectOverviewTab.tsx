...
export default function ProjectOverviewTab({ project, projectId, setProject, isProjectCreator, currentUser, allowEdit=false }: ProjectOverviewTabProps) {
  ...
  const [orgsOwned, setOrgsOwned] = useState<any[]>([]);
  const [orgSelectLoading, setOrgSelectLoading] = useState(false);
  const [orgLinkId, setOrgLinkId] = useState<string>(project?.organizationId || '');

  useEffect(()=> { setOrgLinkId(project?.organizationId || ''); }, [project?.organizationId]);

  // Load organizations owned by current user to allow linking
  useEffect(()=> {
            ...
            {(allowEdit && isProjectCreator) ? (
              <div className="mb-6">
                ...
                <div className="space-y-5">
                  {orgsOwned.length > 0 && (
                    <div>
                      <label className="block text-[11px] font-semibold uppercase tracking-wide text-brand-main mb-1">Link to Organization</label>
                      <div className="flex gap-2 items-center">
                        <select value={orgLinkId} onChange={e=> setOrgLinkId(e.target.value)} disabled={!allowEdit || !isProjectCreator || orgSelectLoading} className="border rounded px-3 py-2 text-sm flex-1">
                      ...
                      <p className="mt-1 text-[10px] text-gray-500">Select one of your organizations to associate with this project.</p>
                    </div>
                  )}
                  <div>
