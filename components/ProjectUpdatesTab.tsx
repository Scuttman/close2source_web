...
interface Props {
  ...
  setProject: React.Dispatch<React.SetStateAction<any>>; // state setter
  projectId: string;                              // public code or fallback id (used for storage paths)
  projectDocId?: string;                          // Firestore document id (authoritative)
  currentUser: any;                               // firebase user
  allowEdit?: boolean;                            // edit mode flag
}

export default function ProjectUpdatesTab({ project, setProject, projectId, projectDocId, currentUser, allowEdit=false }: Props){
  const docId = projectDocId || projectId; // id used for Firestore document operations
  ...
  useEffect(()=>{
    if(!Array.isArray(project?.updates) || !docId) return;
  ...
  return (
    <div ref={updatesPanelRef} className="flex flex-col lg:flex-row gap-8">
      <div className="flex-1 min-w-0">
        ...
        {showComposer && allowEdit && (
          <div className="mb-6 lg:hidden">
            <InlineProjectComposer projectId={projectId} docId={docId} currentUser={currentUser} onPosted={()=> setShowComposer(false)} />
          </div>
        )}
      ...
      <div className="w-full lg:w-72 lg:ml-4 flex-shrink-0 hidden lg:block">
        {showComposer && allowEdit && (
          <div className="mb-6"><InlineProjectComposer projectId={projectId} docId={docId} currentUser={currentUser} onPosted={()=> setShowComposer(false)} /></div>
        )}
        <SidebarSearchTags />
      </div>
...
function InlineProjectComposer({ projectId, docId, currentUser, onPosted }: { projectId:string; docId:string; currentUser:any; onPosted:()=>void }) {
